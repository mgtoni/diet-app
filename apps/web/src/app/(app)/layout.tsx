...
export default function AppLayout({
  ...
  return (
    <div className="min-h-screen bg-cream-bg text-on-surface font-body-md overflow-x-hidden flex flex-col bg-[radial-gradient(#d1d5db_1px,transparent_1px)] [background-size:24px_24px]">
      <DevBypassBanner />
      {/* Top Navigation Bar */}
      <nav className="bg-cream-bg/90 backdrop-blur-sm shadow-sm sticky top-0 z-50 h-16 flex items-center shrink-0">
      ...
      <div className="max-w-[1200px] mx-auto px-gutter py-gutter lg:flex lg:gap-6 flex-1 w-full">
        {/* Sidebar (Desktop) */}
        <aside className="hidden lg:flex flex-col h-[calc(100vh-80px)] py-gutter sticky top-20 w-64 bg-mint-surface rounded-2xl shadow-md shrink-0 border border-outline-variant/30">
        ...
        {/* Main Content Area */}
        <main className="flex-1 lg:max-w-4xl pb-32 lg:pb-0 bg-[#FDF6E3] lg:rounded-2xl shadow-md border border-[#eaddc5] overflow-hidden">
          
          <div className="w-full h-full lg:rounded-2xl">
            {children}
          </div>
        </main>
      </div>
