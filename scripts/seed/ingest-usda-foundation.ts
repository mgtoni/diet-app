...
const DATA_DIR = path.join(__dirname, 'data', 'FoodData_Central_foundation_food_csv_2026-04-30', 'FoodData_Central_foundation_food_csv_2026-04-30');
...
async function ingestUSDAFoundation() {
  console.log('Loading USDA Foundation Data...');
  ...
  // Only ingest actual 'foundation_food' types (skip sample_food, market_acquisition, sub_sample_food which are just intermediate lab records)
  const foundationFoods = foods.filter(f => f.data_type === 'foundation_food');

  console.log(`Loaded ${foundationFoods.length} foundation foods, ${nutrients.length} nutrients, ${foodNutrients.length} food-nutrient links.`);
  ...
  console.log('\nUSDA Foundation Ingestion complete.');
}

ingestUSDAFoundation().catch(console.error);
